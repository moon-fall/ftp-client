...
FTPCMD ftp_cmd[]=
{
	...
	{"rkdir","RKD","Remote directory",do_common_cmd},  
...
void report(struct timeval *start_time,struct timeval *finish_time,int fsize)
{
	...
	dtime=(finish_time->tv_sec-start_time->tv_sec)+(finish_time->tv_usec-start_time->tv_usec)/(1000*1000.0);
	sprintf(outstr,"%-6.2f",dtime);
	
	p=strtok(outstr," ");
	if(p)
		printf("%d bytes receive in %s secs.\n",fsize,p);
	else
		printf("%d bytes receive in %-6.2f secs.\n",fsize,outstr);
}
